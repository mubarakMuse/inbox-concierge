...
export async function getAuthStatus() {
  const res = await fetch(apiUrl('/auth/status'), defaultFetchOpts)
  return parseJson(res)
}

export async function disconnect() {
  const res = await fetch(apiUrl('/auth/disconnect'), { method: 'POST', ...defaultFetchOpts })
  if (!res.ok) throw new Error('Failed to disconnect')
}

export async function deleteAllMyData() {
  const res = await fetch(apiUrl('/auth/delete-all-data'), { method: 'POST', ...defaultFetchOpts })
